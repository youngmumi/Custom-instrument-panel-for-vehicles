...
const byte PulsesPerRevolution = 8;  // Set how many pulses there are on each revolution. Default: 2.
...
volatile unsigned long PeriodBetweenPulses = ZeroTimeout+1000;  // Stores the period between pulses in microseconds.       <- Preriod
                       // It has a big number so it doesn't start with 0 which would be interpreted as a high frequency.
volatile unsigned long PeriodAverage = ZeroTimeout+1000;  // Stores the period between pulses in microseconds in total, if we are taking multiple pulses.
                       // It has a big number so it doesn't start with 0 which would be interpreted as a high frequency.
unsigned long FrequencyRaw;  // Calculated frequency, based on the period. This has a lot of extra decimals without the decimal point.
unsigned long FrequencyReal;  // Frequency without decimals.    <-Freuqeuncy
unsigned long RPM;  // Raw RPM without any processing.    <-RPM
...
unsigned int AmountOfReadings = 5;   //   <-Readings 
...
unsigned long average;  // The RPM value after applying the smoothing.   <-Tachometer
...
void setup()  // Start of setup:
{
  ...
  attachInterrupt(digitalPinToInterrupt(2), Pulse_Event,HIGH);  // Enable interruption pin 2 when going from LOW to HIGH.
